...
class SkillController:
    ...
    def reset(self, action, robot_num):
        ...
        robot = self._env.robots[robot_num]
    ...
    def _get_info(self, robot_num):
        info = self._env._get_skill_info(robot_num)
        robot = self._env.robots[robot_num]
        #maybe change for each robot --> here we also need peg tip and hole center
        #if robot_num == 0:
    ...
    def is_success(self,robot_num):
        info = self._get_info(robot_num)
        return self._cur_skill.is_success(info)

    def done(self, robot_num):
        return self.is_success(robot_num) or (self._num_ac_calls >= self._max_ac_calls)
    ...
    def post_process_reward(self, reward, robot_num):
        if not self.is_success(robot_num):
